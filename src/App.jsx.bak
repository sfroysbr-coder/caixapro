...
function LoginScreen({onLogin,dark}){
  const[user,setUser]=useState(""),pass=useState(""),pw=useState(false),err=useState(""),shake=useState(false),load=useState(false);
  const[pw_,setPw]=pass,[show,setShow]=pw,[error,setErr]=err,[shk,setShk]=shake,[loading,setLoad]=load;
...
export default function App(){
  ...
  const addSupp=async()=>{if(!supf.name){toast$("Nome obrigatório.","#f56565");return;}await supabase.from("suppliers").insert([{id:uid(),...supf,added_by:cu.display_name}]);toast$("Fornecedor cadastrado!");setModal(null);setSupf(FSup);};
