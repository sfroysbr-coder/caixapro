...
const expColor  = d => { if(d===null) return "#555870"; if(d<0) return "#f56565"; if(d<=30) return "#f59e0b"; return "#10b981"; };
...
// ─── PRIMITIVES ───────────────────────────────────────────────────────────────
const IS = {width:"100%",background:"#12141f",border:"1px solid #252840",borderRadius:".45rem",padding:".55rem .8rem",color:"#e8eaf6",fontSize:".84rem",fontFamily:"'DM Sans',sans-serif",outline:"none",boxSizing:"border-box"};

const Modal = ({title,onClose,children,wide,icon}) => (
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",backdropFilter:"blur(8px)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:".75rem"}}>
    <div style={{background:"#0d0f1a",border:"1px solid #252840",borderRadius:"1.1rem",width:"100%",maxWidth:wide?700:500,maxHeight:"94vh",overflowY:"auto"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1rem 1.4rem",borderBottom:"1px solid #1a1d30",position:"sticky",top:0,background:"#0d0f1a",zIndex:1}}>
        <div style={{display:"flex",alignItems:"center",gap:".4rem"}}>
          {icon&&<span style={{color:"#4f5ef0"}}><Icon name={icon} size={15}/></span>}
          <span style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".95rem",color:"#e8eaf6"}}>{title}</span>
        </div>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#666a88",cursor:"pointer",padding:".2rem"}}><Icon name="close" size={16}/></button>
      </div>
...
const Field = ({label,hint,children}) => (
  <div style={{marginBottom:".85rem"}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:".3rem"}}>
      ...
      {hint&&<span style={{fontSize:".65rem",color:"#44475a"}}>{hint}</span>}
    </div>
    {children}
  </div>
);
...
const KCard = ({label,value,sub,color="#4f5ef0",icon}) => (
  <div style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem",padding:".9rem 1rem",position:"relative",overflow:"hidden"}}>
    ...
    <div style={{fontSize:".62rem",color:"#666a88",textTransform:"uppercase",letterSpacing:".07em",marginBottom:".3rem"}}>{label}</div>
    <div style={{fontSize:"1.15rem",fontWeight:700,color:"#e8eaf6",fontFamily:"'Syne',sans-serif",letterSpacing:"-.02em"}}>{value}</div>
...
const Pill = ({label,value,color}) => (
  <div style={{background:"#080a14",borderRadius:".4rem",padding:".4rem .6rem"}}>
    <div style={{fontSize:".58rem",color:"#555870",textTransform:"uppercase",letterSpacing:".05em",marginBottom:".15rem"}}>{label}</div>
    ...
    <div style={{minHeight:"100vh",background:"#07080f",display:"flex",alignItems:"center",justifyContent:"center",padding:"1.25rem",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",maxWidth:380,background:"linear-gradient(160deg,#0e1020,#090b16)",border:"1px solid #252848",borderRadius:"1.25rem",padding:"2rem 1.75rem",boxShadow:"0 30px 80px rgba(0,0,0,.6)",animation:shake?"shake .4s ease":"none"}}>
        <div style={{textAlign:"center",marginBottom:"1.75rem"}}>
          ...
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"1.3rem",color:"#e8eaf6",letterSpacing:"-.02em"}}>CaixaPro</div>
          <div style={{fontSize:".73rem",color:"#44475a",marginTop:".25rem"}}>Gestão · Tirzepatida · v5.0</div>
        </div>
        <div style={{marginBottom:".85rem"}}>
          <label style={{display:"block",fontSize:".68rem",color:"#666a88",marginBottom:".3rem",textTransform:"uppercase",letterSpacing:".07em"}}>Usuário</label>
          <div style={{position:"relative"}}><div style={{position:"absolute",left:".8rem",top:"50%",transform:"translateY(-50%)",color:"#44475a"}}><Icon name="user" size={14}/></div><input value={user} onChange={e=>{setUser(e.target.value);setError("");}} onKeyDown={e=>e.key==="Enter"&&attempt()} placeholder="Digite seu usuário" style={{...li,paddingLeft:"2.3rem"}}/></div>
        </div>
        <div style={{marginBottom:"1.25rem"}}>
          <label style={{display:"block",fontSize:".68rem",color:"#666a88",marginBottom:".3rem",textTransform:"uppercase",letterSpacing:".07em"}}>Senha</label>
          <div style={{position:"relative"}}>
            <div style={{position:"absolute",left:".8rem",top:"50%",transform:"translateY(-50%)",color:"#44475a"}}><Icon name="lock" size={14}/></div>
            ...
            <button onClick={()=>setShowPw(v=>!v)} style={{position:"absolute",right:".7rem",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#44475a",padding:0}}><Icon name={showPw?"eye_off":"eye"} size={14}/></button>
          </div>
        </div>
        ...
        <div style={{textAlign:"center",marginTop:"1.25rem",fontSize:".65rem",color:"#333550"}}>CaixaPro © 2026</div>
      </div>
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        button{transition:opacity .15s;}button:hover{opacity:.85;}
        @keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
        @keyframes shake{0%,100%{transform:translateX(0)}20%{transform:translateX(-8px)}40%{transform:translateX(8px)}60%{transform:translateX(-6px)}80%{transform:translateX(6px)}}
      `}</style>

      {toast&&<div style={{position:"fixed",bottom:"1.25rem",right:"1.25rem",zIndex:999,background:"#0d0f1a",border:`1px solid ${toast.color}50`,borderRadius:".65rem",padding:".65rem 1.1rem",display:"flex",alignItems:"center",gap:".45rem",fontSize:".82rem",color:toast.color,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 8px 30px rgba(0,0,0,.5)",animation:"fadeUp .3s ease",maxWidth:320}}><Icon name="check" size={14}/>{toast.msg}</div>}

      <div style={{minHeight:"100vh",background:"#080a14",fontFamily:"'DM Sans',sans-serif",color:"#e8eaf6"}}>

        {/* TOP BAR */}
        <div style={{background:"#0b0d1a",borderBottom:"1px solid #181a28",padding:".65rem 1.1rem",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
          <div style={{display:"flex",alignItems:"center",gap:".45rem"}}>
            ...
            <div>
              ...
              <div style={{fontSize:".55rem",color:"#44475a",lineHeight:1.2}}>Tirzepatida v5.0</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:".3rem",fontSize:".65rem",color:syncing?"#8b44f0":"#333550"}}><Icon name="sync" size={10}/>{syncing?"...":lastSync||"—"}</div>
            <div style={{display:"flex",alignItems:"center",gap:".4rem"}}>
              ...
              <button onClick={()=>setLogoutC(true)} style={{background:"none",border:"none",cursor:"pointer",color:"#44475a",display:"flex"}}><Icon name="logout" size={13}/></button>
            </div>
          </div>
        </div>

        {/* NAV */}
        <div style={{background:"#0b0d1a",borderBottom:"1px solid #181a28",padding:"0 .5rem",display:"flex",overflowX:"auto"}}>
          {nav.map(n=>(
            <button key={n.id} onClick={()=>{setTab(n.id);setSearch("");setFilterCat("all");}} style={{display:"flex",alignItems:"center",gap:".28rem",padding:".6rem .7rem",background:"none",border:"none",cursor:"pointer",fontSize:".72rem",fontFamily:"'DM Sans',sans-serif",fontWeight:600,color:tab===n.id?"#4f5ef0":"#666a88",borderBottom:tab===n.id?"2px solid #4f5ef0":"2px solid transparent",transition:"all .2s",whiteSpace:"nowrap"}}>
        ...
        <div style={{maxWidth:960,margin:"0 auto",padding:"1rem .85rem"}}>
          ...
          {tab==="dashboard"&&(
            <>
              ...
              {/* Alertas */}
              {(zeroStock.length>0||lowStock.length>0)&&(
                <div style={{background:"#0d0f1a",border:"1px solid #f59e0b30",borderRadius:".75rem",padding:".85rem 1rem",marginBottom:"1rem"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:".85rem"}}>
                {/* Últimas vendas */}
                <div style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem",padding:"1rem"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:".75rem"}}>
                    <div style={{display:"flex",alignItems:"center",gap:".35rem",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".82rem",color:"#c4c8e8"}}><Icon name="sales" size={13}/> Últimas Vendas</div>
                    {canEdit&&<Btn sm v="primary" onClick={()=>setModal("sale")}><Icon name="plus" size={11}/> Nova</Btn>}
                  </div>
                  {sales.length===0?<p style={{color:"#333550",fontSize:".78rem",textAlign:"center",padding:"1.25rem 0"}}>Nenhuma venda.</p>
                  :sales.slice(0,5).map(s=>(
                    <div key={s.id} style={{padding:".45rem 0",borderBottom:"1px solid #151728",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div style={{minWidth:0}}>
                        <div style={{fontSize:".78rem",color:"#c4c8e8",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.product_name}</div>
                        <div style={{fontSize:".65rem",color:"#44475a"}}>{s.date} · {s.quantity}un{s.client_name&&` · ${s.client_name}`}</div>
                      </div>
                ...
                <div style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem",padding:"1rem"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:".75rem"}}>
                    <div style={{display:"flex",alignItems:"center",gap:".35rem",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".82rem",color:"#c4c8e8"}}><Icon name="stock" size={13}/> Estoque</div>
                    {canEdit&&<Btn sm v="info" onClick={()=>setModal("stockEntry")}><Icon name="arrow_up" size={11}/> Entrada</Btn>}
                  </div>
                  {products.length===0?<p style={{color:"#333550",fontSize:".78rem",textAlign:"center",padding:"1.25rem 0"}}>Nenhum produto.</p>
                  :products.slice(0,6).map(p=>(
                    <div key={p.id} style={{padding:".45rem 0",borderBottom:"1px solid #151728",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div style={{minWidth:0}}>
                        <div style={{fontSize:".78rem",color:"#c4c8e8",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{CATEGORY_ICONS[p.category]||"📋"} {p.name}</div>
                        <div style={{fontSize:".65rem",color:"#44475a"}}>{p.code}</div>
                      </div>
                  ...
                  <div style={{marginTop:".65rem",paddingTop:".65rem",borderTop:"1px solid #1a1d30",display:"flex",justifyContent:"space-between",fontSize:".72rem"}}>
                    <span style={{color:"#44475a"}}>Valor total estoque</span>
          ...
          {tab==="vendas"&&(
            <>
              ...
              <div style={{position:"relative",marginBottom:".75rem"}}>
                <div style={{position:"absolute",left:".75rem",top:"50%",transform:"translateY(-50%)",color:"#44475a"}}><Icon name="search" size={14}/></div>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por produto ou cliente..." style={{...IS,paddingLeft:"2.1rem",width:"100%",background:"#0d0f1a"}}/>
              </div>
              <div style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem"}}>
                {sales.filter(s=>s.product_name?.toLowerCase().includes(search.toLowerCase())||s.client_name?.toLowerCase().includes(search.toLowerCase())).length===0
                  ?<p style={{color:"#44475a",textAlign:"center",padding:"2.5rem 0",fontSize:".8rem"}}>Nenhuma venda.</p>
                  ...
                  <div key={s.id} style={{padding:".75rem 1rem",borderBottom:"1px solid #151728",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:".4rem",flexWrap:"wrap",marginBottom:".2rem"}}>
                        <span style={{fontWeight:700,fontSize:".85rem",color:"#e8eaf6"}}>{s.product_name}</span>
                        <Badge color="#8b44f0" sm>{s.quantity} un</Badge>
                        <Badge color="#0891b2" sm>{s.payment_method}</Badge>
                      </div>
                      <div style={{fontSize:".68rem",color:"#44475a"}}>{s.date}{s.client_name&&` · 👤 ${s.client_name}`}{s.notes&&` · ${s.notes}`}</div>
                    </div>
                    <div style={{flexShrink:0,marginLeft:"1rem",textAlign:"right"}}>
                      ...
                      <div style={{fontSize:".65rem",color:"#44475a"}}>{fmt(s.unit_price)}/un</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {tab==="estoque"&&(
            <>
              ...
              <div style={{display:"flex",gap:".4rem",marginBottom:".75rem",overflowX:"auto",paddingBottom:".25rem"}}>
                {cats.map(c=>(
                  <button key={c} onClick={()=>setFilterCat(c)} style={{padding:".3rem .7rem",borderRadius:"99px",border:`1px solid ${filterCat===c?"#4f5ef0":"#252840"}`,background:filterCat===c?"#4f5ef018":"transparent",color:filterCat===c?"#4f5ef0":"#666a88",cursor:"pointer",fontSize:".72rem",fontFamily:"'DM Sans',sans-serif",fontWeight:600,whiteSpace:"nowrap"}}>
                  ...
                  return (
                    <div key={p.id} style={{background:"#0d0f1a",border:`1px solid ${p.stock_qty<=0?"#f5656525":p.stock_qty<=(p.min_stock||5)?"#f59e0b25":"#1a1d30"}`,borderRadius:".75rem",padding:".85rem 1rem"}}>
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:".6rem"}}>
                        <div style={{flex:1}}>
                          <div style={{display:"flex",alignItems:"center",gap:".4rem",flexWrap:"wrap",marginBottom:".25rem"}}>
                            <span style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".88rem",color:"#e8eaf6"}}>{CATEGORY_ICONS[p.category]||"📋"} {p.name}</span>
                          ...
                          <div style={{fontSize:".67rem",color:"#44475a"}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(72px,1fr))",gap:".4rem"}}>
                        {[{l:"Estoque",v:`${p.stock_qty} ${p.unit||"un"}`,c:stkColor(p.stock_qty)},{l:"Mínimo",v:`${p.min_stock||5} ${p.unit||"un"}`,c:"#555870"},{l:"Custo",v:fmt(p.cost_per_unit),c:"#8b8fa8"},{l:"Preço",v:fmt(p.price_per_unit),c:"#10b981"},{l:"Markup",v:fmtPct(p.markup),c:"#4f5ef0"},{l:"Margem",v:fmtPct(p.margin),c:"#8b44f0"}].map(m=>(
          ...
          {tab==="caixa"&&(
            <>
              ...
              <div style={{background:"linear-gradient(135deg,#0e1030,#0d0f1a)",border:"1px solid #2a2d58",borderRadius:".75rem",padding:"1rem",marginBottom:".85rem"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".5rem",marginBottom:".65rem"}}>
                  <div style={{textAlign:"center",background:"#080a18",borderRadius:".5rem",padding:".6rem"}}>
                    <div style={{fontSize:".6rem",color:"#555870",textTransform:"uppercase",marginBottom:".2rem"}}>Receita (100%)</div>
                    <div style={{fontSize:".95rem",fontWeight:700,color:"#10b981",fontFamily:"'Syne',sans-serif"}}>{fmt(cashIn)}</div>
                  </div>
                  <div style={{textAlign:"center",background:"#080a18",borderRadius:".5rem",padding:".6rem"}}>
                    <div style={{fontSize:".6rem",color:"#555870",textTransform:"uppercase",marginBottom:".2rem"}}>Custos</div>
                  ...
                  <div style={{textAlign:"center",background:"#080a18",borderRadius:".5rem",padding:".6rem"}}>
                    <div style={{fontSize:".6rem",color:"#555870",textTransform:"uppercase",marginBottom:".2rem"}}>Lucro líquido</div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".5rem"}}>
                  <div style={{textAlign:"center",background:"#080a18",borderRadius:".5rem",padding:".5rem"}}>
                    <div style={{fontSize:".58rem",color:"#555870",textTransform:"uppercase",marginBottom:".15rem"}}>Markup médio</div>
                    <div style={{fontSize:".88rem",fontWeight:700,color:"#f59e0b",fontFamily:"'Syne',sans-serif"}}>{cashOut>0?fmtPct(avgMarkup):"—"}</div>
                  </div>
                  <div style={{textAlign:"center",background:"#080a18",borderRadius:".5rem",padding:".5rem"}}>
                    <div style={{fontSize:".58rem",color:"#555870",textTransform:"uppercase",marginBottom:".15rem"}}>Receita vendas</div>
                    <div style={{fontSize:".88rem",fontWeight:700,color:"#10b981",fontFamily:"'Syne',sans-serif"}}>{fmt(totalSalesRev)}</div>
                  </div>
                  <div style={{textAlign:"center",background:"#080a18",borderRadius:".5rem",padding:".5rem"}}>
                    <div style={{fontSize:".58rem",color:"#555870",textTransform:"uppercase",marginBottom:".15rem"}}>Valor estoque</div>
                    <div style={{fontSize:".88rem",fontWeight:700,color:"#8b44f0",fontFamily:"'Syne',sans-serif"}}>{fmt(totalStockValue)}</div>
                  </div>
                </div>
              </div>
              <div style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem"}}>
                {cashTx.length===0?<p style={{color:"#44475a",fontSize:".8rem",textAlign:"center",padding:"2.5rem 0"}}>Nenhum lançamento.</p>
                :cashTx.map(t=>(
                  <div key={t.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:".75rem 1rem",borderBottom:"1px solid #151728"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:".84rem",color:"#c4c8e8",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.description}</div>
                      <div style={{fontSize:".67rem",color:"#44475a",marginTop:".1rem"}}>{t.date}{t.category&&` · ${t.category}`}{t.added_by&&` · ${t.added_by}`}</div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:".6rem",flexShrink:0,marginLeft:".85rem"}}>
                      ...
                      {canEdit&&!t.sale_id&&<button onClick={async()=>{await supabase.from("cash_transactions").delete().eq("id",t.id);toast$("Removido.","#f59e0b");}} style={{background:"none",border:"none",cursor:"pointer",color:"#333550"}}><Icon name="trash" size={13}/></button>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {tab==="clientes"&&(
            <>
              ...
              {clients.length===0?<p style={{color:"#44475a",textAlign:"center",padding:"3rem 0",fontSize:".8rem"}}>Nenhum cliente.</p>
              :<div style={{display:"grid",gap:".6rem"}}>
                {clients.map(c=>{
                  const cs=sales.filter(s=>s.client_id===c.id||s.client_name===c.name);
                  return (
                    <div key={c.id} style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem",padding:".8rem 1rem",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
                        ...
                        <div>
                          <div style={{fontWeight:700,fontSize:".86rem",color:"#e8eaf6"}}>{c.name}</div>
                          <div style={{fontSize:".68rem",color:"#44475a"}}>{[c.phone,c.email].filter(Boolean).join(" · ")||"Sem contato"}</div>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:".65rem"}}>
                        {cs.length>0&&<div style={{textAlign:"right"}}>
                          ...
                          <div style={{fontSize:".62rem",color:"#44475a"}}>{cs.length} compra{cs.length!==1?"s":""}</div>
                        </div>}
                        {canEdit&&<div style={{display:"flex",gap:".35rem"}}>
                          ...
                          <button onClick={async()=>{await supabase.from("clients").delete().eq("id",c.id);toast$("Removido.","#f59e0b");}} style={{background:"none",border:"none",cursor:"pointer",color:"#333550"}}><Icon name="trash" size={13}/></button>
                        </div>}
                      </div>
                    </div>
                  );
          ...
          {tab==="produtos"&&(
            <>
              ...
              <div style={{display:"grid",gap:".65rem"}}>
                {products.map(p=>(
                  <div key={p.id} style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem",padding:".85rem 1rem"}}>
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:".6rem"}}>
                      <div>
                        <div style={{display:"flex",alignItems:"center",gap:".4rem",flexWrap:"wrap",marginBottom:".2rem"}}>
                          <span style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".9rem",color:"#e8eaf6"}}>{CATEGORY_ICONS[p.category]||"📋"} {p.name}</span>
                          {p.supplier_name&&<Badge color="#8b44f0" sm>🏭 {p.supplier_name}</Badge>}
                        </div>
                        <div style={{fontSize:".67rem",color:"#44475a"}}>Cód: {p.code}{p.batch&&` · Lote: ${p.batch}`} · {p.units_per_pack>1?`${p.units_per_pack} ${p.unit||"un"}/embalagem`:p.unit||"un"}</div>
                        {p.description&&<div style={{fontSize:".7rem",color:"#666a88",marginTop:".15rem"}}>{p.description}</div>}
                      </div>
                      {(isAdmin||canEdit)&&<div style={{display:"flex",gap:".35rem",flexShrink:0,marginLeft:".75rem"}}>
                        ...
                        {isAdmin&&<button onClick={()=>delProduct(p.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#333550"}}><Icon name="trash" size={14}/></button>}
                      </div>}
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(80px,1fr))",gap:".4rem"}}>
                      {[{l:"Estoque",v:`${p.stock_qty} ${p.unit||"un"}`,c:stkColor(p.stock_qty)},{l:"Custo",v:fmt(p.cost_per_unit),c:"#8b8fa8"},{l:"Preço",v:fmt(p.price_per_unit),c:"#10b981"},{l:"Markup",v:fmtPct(p.markup),c:"#4f5ef0"},{l:"Margem",v:fmtPct(p.margin),c:"#8b44f0"},{l:"Lucro/un",v:fmt(p.profit),c:"#10b981"}].map(m=>(
                        <Pill key={m.l} label={m.l} value={m.v} color={m.c}/>
                      ))}
                    </div>
                  </div>
          ...
          {tab==="usuarios"&&isAdmin&&(
            <>
              ...
              <div style={{background:"#0d0f1a",border:"1px solid #1a1d30",borderRadius:".75rem",overflow:"hidden"}}>
                {appUsers.map(u=>(
                  <div key={u.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:".85rem 1rem",borderBottom:"1px solid #151728"}}>
                    <div style={{display:"flex",alignItems:"center",gap:".75rem"}}>
                      <div style={{width:34,height:34,borderRadius:"50%",background:u.active?"linear-gradient(135deg,#4f5ef0,#10b981)":"#252840",display:"inline-flex",alignItems:"center",justifyContent:"center",fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:".85rem",color:"#fff",flexShrink:0}}>{u.display_name.charAt(0).toUpperCase()}</div>
                      <div>
                        <div style={{display:"flex",alignItems:"center",gap:".4rem"}}>
                          <span style={{fontWeight:700,fontSize:".86rem",color:u.active?"#e8eaf6":"#44475a"}}>{u.display_name}</span>
                          <Badge color={u.role==="admin"?"#f59e0b":u.role==="operator"?"#4f5ef0":"#44475a"} sm>{ROLES[u.role]}</Badge>
                          {!u.active&&<Badge color="#f56565" sm>Inativo</Badge>}
                          {u.id===cu.id&&<Badge color="#10b981" sm>Você</Badge>}
                        </div>
                        <div style={{fontSize:".67rem",color:"#44475a"}}>@{u.username}{u.last_login&&` · ${new Date(u.last_login).toLocaleDateString("pt-BR")}`}</div>
                      </div>
                    </div>
      ...
      {modal==="sale"&&(
        <Modal title="Registrar Venda" onClose={()=>setModal(null)} icon="sales" wide>
          <div style={{background:"#080a14",borderRadius:".5rem",padding:".6rem .85rem",marginBottom:".85rem",fontSize:".75rem",color:"#4f5ef0",display:"flex",gap:".35rem",alignItems:"center"}}><Icon name="info" size={12}/> Venda baixa estoque e lança automaticamente no caixa</div>
          ...
          {sf.product_id&&(
            <div style={{background:"#080a14",borderRadius:".5rem",padding:".75rem",marginBottom:".85rem",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".5rem",textAlign:"center"}}>
              {[{l:"Quantidade",v:`${sf.quantity} un`,c:"#e8eaf6"},{l:"Preço unit.",v:fmt(parseFloat(sf.unit_price)||0),c:"#4f5ef0"},{l:"Total",v:fmt(parseFloat(sf.total_price)||0),c:"#10b981"}].map(m=>(
                <div key={m.l}><div style={{fontSize:".6rem",color:"#555870",textTransform:"uppercase",marginBottom:".15rem"}}>{m.l}</div><div style={{fontSize:"1rem",fontWeight:700,color:m.c,fontFamily:"'Syne',sans-serif"}}>{m.v}</div></div>
              ))}
            </div>
          )}
      ...
      {modal==="stockEntry"&&(
        <Modal title="Entrada de Estoque" onClose={()=>setModal(null)} icon="stock">
          <div style={{background:"#080a14",borderRadius:".5rem",padding:".6rem .85rem",marginBottom:".85rem",fontSize:".75rem",color:"#10b981",display:"flex",gap:".35rem",alignItems:"center"}}><Icon name="arrow_up" size={12}/> Entrada gera saída no caixa (custo de compra)</div>
          ...
          {stockF.product_id&&stockF.qty&&(()=>{
            const prod=products.find(p=>p.id===stockF.product_id);
            const qty=parseInt(stockF.qty)||0;
            return qty>0&&<div style={{background:"#0e1e0e",borderRadius:".5rem",padding:".6rem .85rem",marginBottom:".85rem",fontSize:".78rem",color:"#10b981"}}>✅ +{qty} {prod?.unit||"un"} → novo estoque: {(prod?.stock_qty||0)+qty} {prod?.unit||"un"}</div>;
          })()}
          <div style={{display:"flex",gap:".5rem",justifyContent:"flex-end"}}>
            <Btn v="ghost" onClick={()=>setModal(null)}>Cancelar</Btn>
      ...
      {modal==="supplier"&&(
        <Modal title="Fornecedores" onClose={()=>setModal(null)} icon="supplier" wide>
          ...
          {suppliers.length===0?<p style={{color:"#44475a",textAlign:"center",padding:"2rem 0",fontSize:".82rem"}}>Nenhum fornecedor cadastrado.</p>
          :<div style={{display:"grid",gap:".6rem"}}>
            {suppliers.map(s=>(
              <div key={s.id} style={{background:"#12141f",border:"1px solid #252840",borderRadius:".6rem",padding:".75rem 1rem",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div>
                  <div style={{fontWeight:700,fontSize:".88rem",color:"#e8eaf6"}}>🏭 {s.name}</div>
                  <div style={{fontSize:".7rem",color:"#44475a",marginTop:".15rem"}}>{[s.contact,s.phone,s.email].filter(Boolean).join(" · ")||"Sem contato"}</div>
                ...
                {isAdmin&&<div style={{display:"flex",gap:".35rem"}}>
                  ...
                  <button onClick={async()=>{await supabase.from("suppliers").delete().eq("id",s.id);toast$("Fornecedor removido.","#f59e0b");}} style={{background:"none",border:"none",cursor:"pointer",color:"#333550"}}><Icon name="trash" size={13}/></button>
                </div>}
              </div>
            ))}
          </div>}
      ...
      {modal==="produto"&&(
        <Modal title="Novo Produto" onClose={()=>setModal(null)} icon="product" wide>
          ...
          {pf.cost_per_unit&&pf.price_per_unit&&(()=>{
            const {markup,margin,profit}=calcM(pf.cost_per_unit,pf.price_per_unit);
            return <div style={{background:"#080a14",borderRadius:".5rem",padding:".75rem",marginBottom:".85rem",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".5rem",textAlign:"center"}}>
              {[{l:"Markup",v:fmtPct(markup),c:"#4f5ef0"},{l:"Margem",v:fmtPct(margin),c:"#8b44f0"},{l:"Lucro/un",v:fmt(profit),c:"#10b981"}].map(m=><div key={m.l}><div style={{fontSize:".6rem",color:"#555870",textTransform:"uppercase",marginBottom:".15rem"}}>{m.l}</div><div style={{fontSize:".9rem",fontWeight:700,color:m.c,fontFamily:"'Syne',sans-serif"}}>{m.v}</div></div>)}
            </div>;
          })()}
          <Row>
      ...
      {modal==="editProd"&&editing&&(
        <Modal title="Editar Produto" onClose={()=>{setModal(null);setEditing(null);}} icon="edit" wide>
          ...
          {editing.cost_per_unit&&editing.price_per_unit&&(()=>{
            const {markup,margin,profit}=calcM(editing.cost_per_unit,editing.price_per_unit);
            return <div style={{background:"#080a14",borderRadius:".5rem",padding:".65rem",marginBottom:".85rem",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:".4rem",textAlign:"center"}}>
              {[{l:"Markup",v:fmtPct(markup),c:"#4f5ef0"},{l:"Margem",v:fmtPct(margin),c:"#8b44f0"},{l:"Lucro/un",v:fmt(profit),c:"#10b981"}].map(m=><div key={m.l}><div style={{fontSize:".58rem",color:"#555870",textTransform:"uppercase",marginBottom:".12rem"}}>{m.l}</div><div style={{fontSize:".85rem",fontWeight:700,color:m.c,fontFamily:"'Syne',sans-serif"}}>{m.v}</div></div>)}
            </div>;
          })()}
          <Row>
        ...
        <Modal title="Novo Usuário" onClose={()=>setModal(null)} icon="users">
          ...
          <Row>
            <Field label="Senha *" hint="mín. 6 caracteres">
              <div style={{position:"relative"}}><input type={pwShow.a?"text":"password"} value={uf.password} onChange={e=>setUf(f=>({...f,password:e.target.value}))} placeholder="••••••••" style={IS}/><button onClick={()=>setPwShow(s=>({...s,a:!s.a}))} style={{position:"absolute",right:".7rem",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#44475a",padding:0}}><Icon name={pwShow.a?"eye_off":"eye"} size={13}/></button></div>
            </Field>
            <Field label="Confirmar senha">
              <div style={{position:"relative"}}><input type={pwShow.b?"text":"password"} value={uf.password2} onChange={e=>setUf(f=>({...f,password2:e.target.value}))} placeholder="••••••••" style={IS}/><button onClick={()=>setPwShow(s=>({...s,b:!s.b}))} style={{position:"absolute",right:".7rem",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#44475a",padding:0}}><Icon name={pwShow.b?"eye_off":"eye"} size={13}/></button></div>
            </Field>
          </Row>
      ...
      {modal==="editUser"&&editing&&(
        <Modal title={`Editar · ${editing.display_name}`} onClose={()=>{setModal(null);setEditing(null);}} icon="edit">
          <div style={{background:"#080a14",borderRadius:".45rem",padding:".55rem .85rem",marginBottom:".85rem",fontSize:".75rem",color:"#44475a"}}>Login: <b style={{color:"#666a88"}}>@{editing.username}</b></div>
          <Row>
          ...
          <Field label="Status">
            <div style={{display:"flex",gap:".5rem"}}>
              {[{v:true,l:"✅ Ativo"},{v:false,l:"🚫 Inativo"}].map(o=>(
                <button key={String(o.v)} onClick={()=>setEditing(v=>({...v,active:o.v}))} disabled={editing.id===cu.id} style={{flex:1,padding:".55rem",borderRadius:".45rem",border:`1px solid ${editing.active===o.v?"#4f5ef0":"#252840"}`,background:editing.active===o.v?"#4f5ef018":"transparent",color:editing.active===o.v?"#4f5ef0":"#44475a",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:".82rem"}}>{o.l}</button>
              ))}
            </div>
          </Field>
          <div style={{borderTop:"1px solid #1a1d30",paddingTop:".85rem",marginTop:".25rem",marginBottom:".85rem"}}>
            ...
            <Row>
              <Field label="Nova senha"><div style={{position:"relative"}}><input type={pwShow.c?"text":"password"} value={editing.new_password||""} onChange={e=>setEditing(v=>({...v,new_password:e.target.value}))} placeholder="••••••••" style={IS}/><button onClick={()=>setPwShow(s=>({...s,c:!s.c}))} style={{position:"absolute",right:".7rem",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#44475a",padding:0}}><Icon name={pwShow.c?"eye_off":"eye"} size={13}/></button></div></Field>
              <Field label="Confirmar"><div style={{position:"relative"}}><input type={pwShow.d?"text":"password"} value={editing.new_password2||""} onChange={e=>setEditing(v=>({...v,new_password2:e.target.value}))} placeholder="••••••••" style={IS}/><button onClick={()=>setPwShow(s=>({...s,d:!s.d}))} style={{position:"absolute",right:".7rem",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#44475a",padding:0}}><Icon name={pwShow.d?"eye_off":"eye"} size={13}/></button></div></Field>
            </Row>
          </div>
          <div style={{display:"flex",gap:".5rem",justifyContent:"space-between"}}>
      ...
      {/* LOGOUT */}
      {logoutC&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",backdropFilter:"blur(8px)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"1.25rem"}}>
          <div style={{background:"#0d0f1a",border:"1px solid #252840",borderRadius:"1rem",padding:"1.75rem",maxWidth:300,width:"100%",textAlign:"center"}}>
            <div style={{color:"#4f5ef0",marginBottom:".85rem"}}><Icon name="logout" size={32}/></div>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".95rem",color:"#e8eaf6",marginBottom:".4rem"}}>Sair do sistema?</div>
            <div style={{fontSize:".78rem",color:"#44475a",marginBottom:"1.25rem"}}>Precisará fazer login novamente.</div>
